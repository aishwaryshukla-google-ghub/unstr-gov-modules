...
def describe_iceberg_table(
    ...
    """
    Queries official BigLake Iceberg REST Catalog Table Describe API:
    ...
    candidate_tbl_urls = [
        f"https://biglake.googleapis.com/iceberg/v1/restcatalog/projects/{target_project}/catalogs/{catalog_name}/namespaces/{namespace}/tables/{table_name}?alt=json",
        f"https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{target_project}/catalogs/{catalog_name}/namespaces/{namespace}/tables/{table_name}?alt=json",
        f"https://biglake.googleapis.com/iceberg/v1/projects/{target_project}/catalogs/{catalog_name}/namespaces/{namespace}/tables/{table_name}?alt=json",
        f"https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{target_project}/catalogs/{catalog_name}/namespaces/{namespace}/tables/{table_name}",
    ]
    ...
    for url in candidate_tbl_urls:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                table_result["error"] = f"Table '{table_name}' in namespace '{namespace}' not found (HTTP 404)."
            else:
...
def list_catalog_namespaces_and_tables(
    ...
    1. Inspects Catalog Metadata & Refresh Status
    2. Lists all Namespaces (Schemas) inside the Catalog
    3. Lists all Tables inside each discovered Namespace
    ...
    # 1. Query Catalog Metadata & Refresh Status
    ...
    result["api_endpoints_called"].append({"action": "get_catalog", "url": cat_url})
    ...
    try:
        ...
        if cat_resp.status_code == 200:
            ...
            result["errors"].append(f"Catalog describe returned HTTP {cat_resp.status_code}: {cat_resp.text}")
    except Exception as e:
        result["errors"].append(f"Failed to query catalog metadata: {str(e)}")

    # 2. Query Namespaces (Schemas) API
    ns_candidate_urls = [
        f"https://biglake.googleapis.com/iceberg/v1/restcatalog/projects/{target_project}/catalogs/{catalog_name}/namespaces?alt=json",
        f"https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{target_project}/catalogs/{catalog_name}/namespaces?alt=json",
        f"https://biglake.googleapis.com/iceberg/v1/projects/{target_project}/catalogs/{catalog_name}/namespaces?alt=json"
    ]
    ...
    # 3. Query Tables in each Namespace
    total_tables = 0
    tables_by_namespace = {}

    for ns in result["namespaces"]:
        tbl_candidate_urls = [
            f"https://biglake.googleapis.com/iceberg/v1/restcatalog/projects/{target_project}/catalogs/{catalog_name}/namespaces/{ns}/tables?alt=json",
            f"https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{target_project}/catalogs/{catalog_name}/namespaces/{ns}/tables?alt=json",
            f"https://biglake.googleapis.com/iceberg/v1/projects/{target_project}/catalogs/{catalog_name}/namespaces/{ns}/tables?alt=json"
        ]
    ...
    # 4. Describe Specific Table (e.g. 'addresses')
